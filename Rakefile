namespace :test do
  ...
  task :ios => :prepare do
    $ios_success = system("xcodebuild -workspace AFNetworking.xcworkspace -scheme 'iOS Tests' -sdk iphonesimulator -configuration Release test")
  end

  desc "Run the AFNetworking Tests for Mac OS X"
  task :osx => :prepare do
    $osx_success = system("xcodebuild -workspace AFNetworking.xcworkspace -scheme 'OS X Tests' -sdk macosx -configuration Release test")
  end
end
